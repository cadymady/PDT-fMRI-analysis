...
cd(sub_path)         % go back home
 ...
 cd('Physio')
    ...
    for pf = 1:length(physio_files)
        if exist([sub_path, '\Physio\', physio_files{pf}])
            % store path of physio file in cardiac
            matlabbatch{2}.spm.tools.physio.log_files.cardiac = {[sub_path, '\Physio\', physio_files{pf}]};
        else
            error_msg = ['Physio file does not exist. Something wrong with path? \r\n'...
                [sub_path, '\Physio\', cur_sub,'_',session,'_',num2str(pf),'.puls']];
        end
    end
