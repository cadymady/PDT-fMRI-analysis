...
% revision: Katharina Dücker & Francesca Chiarello
% date  : 5.10.2017
...
try
    ...
    for ii = 1 : length(P.cur.choice)
        ...
        if ~(P.cur.choice(ii) > 0 && P.cur.choice(ii) < 5) 
            onsets_cell{nn}(7) = {[cell2mat(onsets_cell{nn}(7)) P.t.cur_trial.stim_on(ii) + time_corr_second_run]};
            durations_cell{nn}(7) = {[cell2mat(durations_cell{nn}(7)) P.t.cur_trial.stim_on(ii+1)-P.t.cur_trial.stim_on(ii)]};
            continue
        end
    ...
    cd(cur_fold{1})     % access first folder and load Preprocessing file
    ...
    for nr = 1:num_run
        ...
        if physio_inc
            ...
            cd(cur_fold)
            matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg{nr} = cellstr(spm_select('FPList',pwd,'^rp'));
        end
